...
def iconPlot(ifile,ofile,otype,varname,vartype='scalar',opts=[])
  ...
  cmd   ="ncl #{SRC[0]} 'altLibDir=\"#{libdir}\"' '#{varIdent}=\"#{varname}\"' 'iFile=\"#{ifile}\"' 'oFile=\"#{ofile}\"' 'oType=\"#{otype}\"' #{opts.join(' ')} 'cdo=\"#{CDO}\"'"
  puts cmd
  sh cmd
...
task :test_sections do
  images = []
  secopts = ["'secLC=(/0,80/)'","'secRC=(/0,-80/)'","showSecMap=True","secPoints=100"]
  COMPARISON.each {|itype,ifile|
    ofile = "test_section_#{itype.to_s}"
    if itype == :atm
      scalarPlot(ifile,ofile+'h',OFMT,DEFAULT_VARNAME,secopts +["'atmLev=\"h\"'","'tStrg=\"atm: height levels\"'"]);images << ofile+'h'
      scalarPlot(ifile,ofile+'p',OFMT,DEFAULT_VARNAME,secopts +["'atmLev=\"p\"'","'tStrg=\"atm: pressure levels\"'"]);images << ofile+'p'
      scalarPlot(ifile,ofile+'m',OFMT,DEFAULT_VARNAME,secopts +["'atmLev=\"m\"'","'tStrg=\"atm: model levels\"'"]);images << ofile+'m'
    else
      scalarPlot(ifile,ofile,OFMT,DEFAULT_VARNAME,secopts+["'tStrg=\"oce: ocean depth\"'"]); images << ofile
    end
  }
  images.map! {|i| i+= ".#{OFMT}"}
  show(*images)
...
COMPARISON.each {|itype,ifile|
  ...
  task "test_overlay_3d_#{itypeS}".to_sym do
    defaultPlot(ifile,"test_overlay_3d_#{itypeS}",["'vecVars=\"#{vecvars}\"'",
                "'mapType=\"ortho\"'","'atmLev=\"m\"' levIndex=#{levIndex}","centerLat=90"])
    defaultPlot(ifile,"test_overlay_stream_3d_#{itypeS}",["'vecVars=\"#{vecvars}\"'",
                "'mapType=\"ortho\"'","'atmLev=\"m\"' levIndex=#{levIndex}","centerLat=-90","streamLine=True"])
  end
  desc "overlay plot from 2d variable (#{itypeS})"
  task "test_overlay_2d_#{itypeS}".to_sym do
    ofile = "test_overlay_2d_#{itypeS}"
    image = iconPlot(ifile,ofile,OFMT,var2d,'scalar',["'vecVars=\"#{vecvars}\"'",
                "'mapType=\"ortho\"'","'atmLev=\"m\"' levIndex=#{levIndex}","centerLon=120","vecRefLength=0.04"])
    show(image)
    ofile = "test_overlay_stream_2d_#{itypeS}"
    image = iconPlot(ifile,ofile,OFMT,var2d,'scalar',["'vecVars=\"#{vecvars}\"'",
                "'mapType=\"ortho\"'","'atmLev=\"m\"' levIndex=#{levIndex}","centerLon=120","streamLine=True"])
    show(image)
  end
}
...
task :test_reg_section_oce do
  ...
  secopts = ["'secLC=(/0,80/)'","'secRC=(/0,-80/)'","showSecMap=False","secPoints=100"]
  ofile = "test_section_ice"
  scalarPlot(OCE_REGPLOT_TEST_FILE,ofile,OFMT,DEFAULT_VARNAME,secopts)
  ofile += ".#{OFMT}"
  show(ofile)
end
desc "plot section of ocean and atmosphere (height, pressure and model level)"
...
["'colormap=(/ \"white\",\"black\",\"firebrick\",\"peachpuff\",\"orangered\",\"navyblue\",\"peru\",\"yellow\",\"wheat1\",\"gray55\",\"thistle\",\"coral\",\"dodgerblue\", \"seagreen\",\"maroon\",\"gold\",\"turquoise\",\"mediumorchid\" /)'"])
end
#==============================================================================
# Test collections
desc "Run all tests"
